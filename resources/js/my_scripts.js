...
function loadStatsPage(){
	...
	for(var i = 2, row; row = table.rows[i]; i++){
		opponent = row.cells[1].innerHTML;
		homeScore = parseInt(row.cells[2].innerHTML, 10);
		awayScore = parseInt(row.cells[3].innerHTML, 10 );

		if(homeScore > awayScore){
			totalWins++;
			document.getElementById("wins").innerHTML = totalWins;
			buffsWin = true;
			console.log(totalWins + "Buffs win!")
		}
			...
			buffsWin = false;
			console.log(totalLosses + "Buffs Loss ;(")
		}

		if(buffsWin){
			row.cells[4].innerHTML = "CU Buffs";
		}

		else {
			row.cells[4].innerHTML = opponent;
		}
	}
}
...
function loadPlayersPage(){
	...
	for(var i = 0; i < players.length; i++){
		var tag = "<a href=\"#\" onclick=\"switchPlayers(" + i + ")\">" + players[i].name + "</a><br>";
